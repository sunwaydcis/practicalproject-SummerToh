...
class PersonOverviewController():
  ...
  def initialize() =
    ...
    firstNameLabel.text <== myText.text //publisher, string property
    showPersonDetails(None) //clear to empty string
    personTable.selectionModel.value.selectedItem.onChange( //object property
  ...
  private def showPersonDetails(person: Option[Person]): Unit =
    person match
      case Some(person) =>
        ...
        birthdayLabel.text <== Bindings.createStringBinding( //customize
          () =>{ person.date.value.asString } , person.date
        )

      case None =>
        // Person is null, remove all the text.
        firstNameLabel.text.unbind() //unbind removes all the findings, can set the value, not subscribe
  ...
  def handleDeletePerson(action: ActionEvent) =
    ...
    if (selectedIndex >= 0) then
      ...
      // Nothing selected.
      val alert = new Alert(AlertType.Warning): //error,warning,none,information.confirmation
